...
require(['react', 'slider-input'], function(React, SliderInput) {
	window.onload = function() {
		var containers = document.querySelectorAll('.slider');
		var valueCons = document.querySelectorAll('.value');


		React.render(<SliderInput indicate={false} max={1} step={0.01}
			onChange={function(val) {valueCons[0].innerText=val;}}
			onChangeStop={function(val) {console.log(val);}}></SliderInput>, containers[0]);
		...
		React.render(<SliderInput indicate={true} percentageMode={true} initialProgress={30} onChange={function(val) {valueCons[3].innerText=val;}}></SliderInput>, containers[3]);
